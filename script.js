...
userData.forEach((student) => {
    ...
    // Create the cells for the student's properties
    ...
    if (student.passing) {
      ...
      passingCell.appendChild(document.createTextNode("failed"));
    }
...
let input=document.getElementById("input");
    search.addEventListener("click", (event) => {
        tbody.innerHTML="";
         let in1=input.value;
         //Search on the basis of firstname or lastname or email
        let matchingStudents = userData.filter((student)=> {
            let name=student.first_name+student.last_name;
            if(student.first_name.toLowerCase()==in1.toLowerCase()){
                return 1;
            }
            else if(student.last_name.toLowerCase()==in1.toLowerCase()){
                return 1;
            }
            else if(student.email==in1.toLowerCase()){
                return 1;
            }
            else if(name.toLowerCase()==in1.toLowerCase()){
                return 1;
            }
            else{
                return 0;
  ...
  //1.Sort the fullname in ascendending order 
    ...
    asc.addEventListener("click", (event) => {
        ...
        // Loop through the sorted array and add the rows to the table
      ...
      //Sort Z->A
      dsc.addEventListener("click", (event) => {
      ...
      marks.addEventListener("click", (event) => {
        ...
        const sortedData = userData.sort((a, b) => a.marks - b.marks);
      ...
      pass.addEventListener("click", (event) => {
        // only passing student 
           
        let sortedData = userData.filter((student)=> {
       ...
       //Sort by Class ascending order

       let class1=document.getElementById("class");
      class1.addEventListener("click", (event) => {
        // Sort the array by marks in ascending order
        const sortedData = userData.sort((a, b) => a.class - b.class);
      ...
      gender.addEventListener("click", (event) => {
          ...
          let sortedData1 = userData.filter((student)=> {
        ...
        const sortedData = sortedData1.sort((a, b) => {
            const male = `${a.gender} ${b.gender}`;
            const female= `${b.gender} ${b.gender}`;
            if (male < female) {
              return -1;
            } else if (male > female) {
              return 1;
